...
public class EntryPoint {
	...
	public int gridSearch(
		...
		for (int serverId = 0; serverId < dataSets.size(); serverId++) {
			...
			ModelTrainer modelTrainer = new ModelTrainer(config.crossValidationFold());
			SingleServerMasterModel model = modelTrainer.trainWithGridSearch(dataSet);
...
//		evaluator.generateReport(new File("training-report.csv"));
	...
	public int train(
		...
		for (int serverId = 0; serverId < dataSets.size(); serverId++) {
			...
			ModelTrainer modelTrainer = new ModelTrainer(config.crossValidationFold());
			SingleServerMasterModel model = modelTrainer.trainWithGivenParameters(dataSet, config.modelParameters());
			modelTrainer.generateTrainingReport(new File("grid-search-" + serverId + ".csv"));
